...
var htmlconetnt = {
  ...
  ajaxloadmedialist: function(itime){
    ...
    $.ajax({
      ...
      success:function(data){
        popup.closeloading();
        if(data.code == '10'){
          $(htmlconetnt.medialistobj['0']).append(htmlconetnt.mdeiacontent(data.list));
        }
  ...
  mdeiacontent: function(data){//AAAAAAAAAAAAA
    var a = "";
    for (var x in data){
      ...
      a += '<div class="msg-mediacard" newstime="'+updatetime+'">';
      a += '<input type="radio" class="msgnewradio"  name="msgnewradio" id="r'+x+'" value="'+x+'" />';
      ...
      a += '</ul>';
      a += '</div>';
      a += '<div class="msgnewbutton"><label for="r'+x+'" class="label"><i class="fa fa-check"></i><i class="msg-check">Pick</i></button></div>';
      a += '</div>';
    }
    return a;
  },
  mediamessage: function(){
    if(!htmlconetnt.medialistobj){
      var a = '<div class="msg-conetnt"></div>';
      a += '<button class="msg-renew btn btn-default btn-xs"><i class="fa fa-chevron-down"></i></button>';
      htmlconetnt.medialistobj = $(a);
      htmlconetnt.ajaxloadmedialist();
    }
    $(htmlconetnt.medialistobj['1']).click(function(){
      var itime = $(htmlconetnt.medialistobj['0']).children('.msg-mediacard').last().attr('newstime');
      htmlconetnt.ajaxloadmedialist(itime);
    });
    return htmlconetnt.medialistobj;
  },
