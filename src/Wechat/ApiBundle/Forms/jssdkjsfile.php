...
class jssdkjsfile extends FormRequest{

    public function rule(){
      return array(
        'jsid' => new Assert\NotBlank(),
    ...
    public function dealData(){
      $check = $this->checkJssdkId();
      if($check['code'] != '10')
        return $check;
      $jssdk = $this->container->get('my.Wechat')->getJsSDK($this->getdata['refererurl']);
      $jscode = array(
        'debug' => (isset($this->getdata['debug']) && $this->getdata['debug'])?true:false,
...
EOF;
    ...
    public function checkJssdkId(){
      $redis = $this->container->get('my.RedisLogic');
      $jsredis = 'jssdk:'.$this->getdata['jsid'];
      $jscontct = $redis->getString($jsredis);
      if(!$jscontct && $this->getdata['jsid'] != '60c4349e-c302-4313-9fa8-37a8ebd59853'){
        $info = $this->container->get('my.dataSql')->searchData(array('jsfilename' => $this->getdata['jsid']) ,array(), 'wechat_jssdk');
        if($info && isset($info[0])){
          $jscontct = json_encode($info['0'], JSON_UNESCAPED_UNICODE);
          $redis->setString($jsredis, $jscontct, 7200);
        }else{
          return array('code' => '12', 'msg' => 'not found this jsid');
        }
      }
      $jscontct = json_decode($jscontct, true);
      preg_match_all("/^http[s]{0,1}:\/\/([^\/]*)\/.*/", $this->getdata['refererurl'], $newpath, PREG_SET_ORDER);
      $domain = isset($newpath['0']['1'])?$newpath['0']['1']:'';
      if($this->getdata['jsid'] == '60c4349e-c302-4313-9fa8-37a8ebd59853')
        $jscontct['domain'] = isset($_SERVER['HTTP_HOST'])?$_SERVER['HTTP_HOST']:'';
      if($domain === $jscontct['domain'])
        return array('code' => '10', 'msg' => 'jsid allow');
      return array('code' => '13', 'msg' => 'jsid domain not allow');
    }
}
